jQuery(document).ready(function () {
    ...
    var choice=null;
     ...
     const setInMotion = () => {
       const startedMotion = setInterval(frame, 100);
       frame();
     };
    ...
    const rpsCompare = () => {
      if (player1.choice === "rock") {
        if (player2.choice === "rock") {
          ...
          jQuery("#winOrLoseOne").html(
            player1.name + " wins!  Rock beats Scissors!"
          );
          database
            .ref()
        ...
        if (player2.choice === "rock") {
          ...
          jQuery("#winOrLoseOne").html(
            player2.name + " wins! Scissors beats Paper!"
          );
          database
            .ref()
            .child("/players/player1/loss")
        ...
        if (player2.choice === "rock") {
          ...
          jQuery("#winOrLoseOne").html(
            player1.name + " wins! Scissors beats Paper!"
          );
          database
            .ref()
            .child("/players/player1/win")
    ...
    jQuery("#register").on("click", function (event) {
        ...
        if ((jQuery("#username").val().trim() !== "") && !(player1 && player2)) {
            ...
            const msg = yourPlayerName + " has joined!";
            console.log(msg);
    ...
    database.ref("/turn/").on("value", function (snapshot) {
        if (snapshot.val() === 1) {
            ...
            if (player1 && player2) {
                ...
                jQuery("#make-choice").html("<h4>" + player2Name + ", make your choice!</h4>");
            }
            if (snapshot.child("/outcome/").exists()) {
    ...
    database.ref("/chat/").on("child_added", function (snapshot) {
        ...
        let chatEntry = jQuery("<br><br><div>").html(chatMsg);
    ...
    jQuery("#chat-send").on("click", function (event) {
        ...
        if ((yourPlayerName !== "") && (jQuery("#player-chat").val().trim() !== "")) {
            let msg = yourPlayerName + ": " + jQuery("#player-chat").val().trim();
    ...
    database.ref("/outcome/").on("value", function (snapshot) {
        let outcome = snapshot.val();
        let inputPlayer1Choice = "";
        let inputPlayer2Choice = "";
        let firstLetter=(player1.choice).slice(0,1);
        let capitalize=firstLetter.toUpperCase();
        let rest = (player1.choice).substring(1, (player1.choice).length);
        let firstLetterTwo = (player2.choice).slice(0, 1);
        let capitalizeTwo = firstLetterTwo.toUpperCase();
        let restTwo = (player2.choice).substring(1, (player2.choice).length);
        console.log("choice: " + inputPlayer1Choice);
        console.log("choice2: " + inputPlayer2Choice);   
        console.log("outcome has changed");
        jQuery("#winOrLoseOne").html(snapshot.val());
        let showAnswerOne = "./" + capitalize + rest + ".png";
        let showAnswerTwo = "./" + capitalizeTwo + restTwo + ".png";
...
setTimeout(showAnswer,1000);
    ...
    jQuery("#tools").on("click", ".btnOne", function (event) {
        ...
        if (player1 && player2 && (yourPlayerName === player1.name) && (turn === 1))   {
            ...
            let choice = jQuery(this).val().trim();
